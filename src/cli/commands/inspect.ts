...
export const statusCommand: CommandSpec = {
  ...
  description: [
    ...
    "field order are stable, and new fields are only ever appended:",
    "",
    "  version           1",
    "  base.branch       <name|empty>          base.head     <oid|empty>",
    "  overlay.branch    <name|empty>          overlay.head  <oid|empty>",
    "  overlay.upstream  <name|empty>          overlay.ahead <n>   overlay.behind <n>",
    "  detached          0|1                   sync-in-progress 0|1",
    "  detach     <when> <oid|empty> <paths> <complete>",
    "  problems          <n>",
    "  file       <kind> <staged> <dirty> <missing> <upstream> <path>",
    "  base       <XY> <path>                  git's own porcelain-v1 characters",
    "  base-orig  <XY> <path>                  rename source; follows its `base` record",
    "  sync-pending  <path>                    `overgit sync` has work here",
    "  pull-blocked  <path>                    a base `git pull` will abort on this path",
    "",
    "`file` fields: kind is add|override|delete; staged, dirty and missing are 0 or 1;",
    "upstream is same|changed|deleted|added (the base now tracks it)|unknown.",
    "",
    "`detach` is present only while detached: when the overlay was unmounted (ISO 8601), the",
    "overlay commit it was unmounted at, how many paths it owned, and whether that detach ran",
    "to completion — `complete 0` means the work-tree is only half unmounted.",
    "",
